...
- **Optimizer request format**: `type: "mtg_single"` with `name`, `card_id`, `set_code` (only when `allowed_sets` set), `condition_ids` (all conditions ≥ min_condition), `finish_ids` (`NF`/`FO`/both), `quantity_requested`. As of ~2026-08 the API requires one of `card_id`/`mtgjson_id`/`set_code`+`collector_number` per item (confirmed via `/api/docs/v1/openapi.json` on the live site — `name` is documented as "informational only; not used to match printings", and `_build_optimizer_payload()` hasn't changed since 2026-06-25 per `git log`, so this is a ManaPool-side change, not a regression). `card_id` = Scryfall **oracle_id** (stable across every printing), resolved via `ScryfallBulk.get_oracle_id()` — per the API docs, `card_id` means "any interchangeable printing of it may be substituted", which is what preserves the site's "Any printings" search behavior; `mtgjson_id` by contrast pins one specific printing. Verified empirically: captured the site's own optimizer network traffic and cross-referenced a `products_mtg_single.card_id` value against `data/tcgtracking/*_cards.json`'s `mtgjson_uuid` field for the same card — confirms ManaPool's internal per-printing id is mtgjson-based, distinct from the public API's card-level `card_id`. Items whose name doesn't resolve to an oracle_id in the bulk cache are skipped (logged as a warning) rather than sent with no identifier, since one bad item 400s the whole batch.
